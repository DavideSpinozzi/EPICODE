function Confronto(numero1: number, numero2: number): string {
  ...
  const differenza1 = Math.abs(numero1 - numeroCasuale);
  const differenza2 = Math.abs(numero2 - numeroCasuale);
...
const Giocatore1: number = 5;
const Giocatore2: number = 64;
const risultato: string = Confronto(Giocatore1, Giocatore2);
